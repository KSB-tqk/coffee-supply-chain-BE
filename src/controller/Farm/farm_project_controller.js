...
const farmProjectController = {
    ...
    updateFarmProject: async (req, res) => {
        try {
            ...
            if (!farmProject) return res.status(400).json({ msg: "This farm project doesn't exist" });

            else if(!landProject) return res.status(400).json({ msg: "This land doesn't exist" });

            else if(!seedProject) return res.status(400).json({ msg: "This land doesn't exist" });
                ...
                return res.status(400).json({ msg: "Total harvest must be greater than 0" });
            } else {
    ...
    getAllFarmProjects: async (req, res) => {
        try {
            ...
            const farmProjects = await FarmProjectModel.find({ farmId: id }).populate(["land", "seed"]).exec();
    ...
    getFarmProject: async (req, res) => {
        try {
            ...
            const farmProject = await FarmProjectModel.findById(id).populate(["land", "seed"]).exec();
